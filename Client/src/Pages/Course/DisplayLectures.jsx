           ...
           {  lectures && lectures.length > 0 ? (
            ...
            <div className='flex justify-center w-full gap-10 mt-10  '>
              ...
              <div className='py-5 w-md  p-2 rounded-lg shadow-[0_0_10px_black]'>
                ...
                <div>
                  <h2 className='text-green-500 text-xl '>  {""} {lectures?.[currentVideo]?.title}</h2>
                  <p className='text-gray-500 text- line-clamp-4'> {""} {lectures?.[currentVideo]?.description}</p>
                </div>
              </div>
              ...
              <ul className='w-md p-2 rounded-lg shadow-[0_0_5px_black]'>
                <li>
                  ...
                  {role === "ADMIN" && (
                    <button className='bg-green-500 px-3 py-1 roundex-md hover:bg-green-600 transition-all ease-in-out duration-300'>Add Lecture</button>
                  )}
                </li>
                {
                  lectures && 
                      ...
                      <li className='space-y-3'
                       key={lecture._id}>
                        <p className='cursor-pointer ' onClick={()=> setCurrentVideo(idx)}>
                         ...
                         {role === "ADMIN" && (
                    <button
                    onClick={()=> handelLectureDeletion(state?._id , lecture?._id)}
                     className='bg-red-500 px-3 py-1 roundex-md hover:bg-red-600 transition-all ease-in-out duration-300'>Delete Lecture</button>
                  )}
                       </li>
                    )
                  })
